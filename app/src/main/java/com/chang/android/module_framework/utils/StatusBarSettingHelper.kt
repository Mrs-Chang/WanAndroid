...
import com.chang.android.module_framework.utils.OSUtils.isEMUI3_x

object StatusBarSettingHelper {
    /**
     * 沉浸式状态栏
     * */
    fun immersiveStatusBar(activity: Activity, dark: Boolean, fitSystemWindows: Boolean = false) {
        activity.window.navigationBarColor = Color.TRANSPARENT
        activity.window.statusBarColor = Color.TRANSPARENT
        val controller = WindowInsetsControllerCompat(activity.window, activity.window.decorView)
        controller.isAppearanceLightStatusBars = !dark
        WindowCompat.setDecorFitsSystemWindows(activity.window, fitSystemWindows) // 打开沉浸式
    }
